...
int _tmain(int argc, TCHAR* argv[])
{
	...
	BOOL fSuccess;
	LPTSTR PIPE_SERVER_NAME = TEXT("\\\\.\\Arkanoid\\pipe");
	...
	for (i = 0; i < MAXPLAYERS; i++)
	{
		...
		Pipe[i].hPipeInst = CreateNamedPipe(
			...
			sizeof(PLAYERS),   // output buffer size 
			sizeof(PLAYERS),   // input buffer size 
	...
	DWORD WINAPI ServerInputPipes() {

		PLAYERS pAction;

		while (LIVE == true) {
			PrintPlayers();
			pAction = ReceiveRequestPipe();
			HandleAction(pAction);
		}
